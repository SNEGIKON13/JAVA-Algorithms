...
+ **Задача 6**
     1. Условие: Добавьте в класс BST метод height () для выч:исления высоты дерева. Разработайте
две реализации: рекурсивный метод (требующий линейного времени и
памяти, пропорциональной высоте) и метод наподобие sizeO, для работы которого
нужно дополliительное поле в каждом узле дерева (требующий линейного
объема памяти и константного времени на залрос) ..
...
+ **Задача 17**  
     1. Условие: Hapиcyf'rre последовательность деревьев бинарного поиска , которые получаются
при поочередном удалении ключ.ей из дерева из упражнения 3.2.1 в порядке их
вставки.
...
+ **Задача 25** 
     1. Условие: Идеальная балансировКll. Напишите программу, которая вставляет набор юuочей
в первоначально пустое ДБП так, что полученное дерево эквивалентно бинарному
поиску - т.е. последователыюстъ сравнений, выполняемых при поиске
любого ключа в ДБП, совпадает с последовательностью сравнений, ВЬШОJ]]-{Яемых:
...
+ **Задача 28** 
     1. Условие: Програ.ммпое кеширование. Добавьте в класс вsт сохранение в переменной экземпляров
узла, к которому было последнее обращение, чтобы в неrо можно
бьшо попасть за константное время , если операция. put О или get О повторно
